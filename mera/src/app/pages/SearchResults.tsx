    ...
    <div className="h-full min-h-0 max-h-full overflow-y-auto overscroll-contain">
      ...
      <SectionCard title="Results" subtitle={query ? `Ranked matches for ${query}` : 'Enter at least two characters to search'}>
        {loading ? (
          ...
          <div className="px-4 py-8 text-sm text-[#6b7280]">Enter at least two characters to search MERA records.</div>
        ) : results.length ? (
          ...
          <div className="px-4 py-8 text-sm text-[#6b7280]">No matching regulator records found for "{query}".</div>
        )}
      </SectionCard>
      </div>
    </div>
