...
const ALERT_NOTIFICATION_ICON = "/smartlink.png";
...
function matchMobileRoute(pathname) {
  ...
  if (pathname === "/m/reservations")
    return { name: "reservations", params: {} };
  ...
  if (pathname === "/m/wallet/send-credit")
    return { name: "send-credit", params: {} };
  if (pathname === "/m/mera/report") return { name: "mera-report", params: {} };
  if (pathname === "/m/mera/report/success")
    return { name: "mera-report-success", params: {} };
...
function activeTabForRoute(routeName) {
  ...
  if (
    routeName === "stations" ||
    routeName === "station-details" ||
    routeName === "directions"
  )
    return "home";
  if (routeName === "activity" || routeName === "queue-status")
    return "activity";
...
function toPushSubscriptionPayload(subscription) {
  ...
  return {
    endpoint,
    expirationTime:
      raw?.expirationTime !== undefined && raw?.expirationTime !== null
      ...
      setCelebration({
        ...
        variant:
          String(detail.variant || "")
            .trim()
            .toLowerCase() || "wallet-transfer",
        title:
          String(detail.title || "Funds Received").trim() || "Funds Received",
        subtitle:
          String(detail.subtitle || "SmartLink credit received").trim() ||
          "SmartLink credit received",
        transferMode:
          String(detail.transferMode || "")
            .trim()
            .toUpperCase() || "NORMAL",
      });
...
function MobileApp({ theme = "light", onThemeChange }) {
  ...
  const [activeQueueJoinId, setActiveQueueJoinId] = useState(() =>
    getStoredActiveQueueJoinId(),
  );
  ...
  const [stationsData, setStationsData] = useState(() =>
    stationsApi.isApiMode() ? [] : mockStations,
  );
  ...
  const previousStationsSnapshotRef = useRef(
    stationsApi.isApiMode() ? [] : mockStations,
  );
  ...
  const stopTransitionLoader = useCallback((delayMs = 0) => {
  ...
  const startTransitionLoader = useCallback((words = APP_BOOT_WORDS) => {
    ...
    setLoaderWords(
      Array.isArray(words) && words.length ? words : APP_BOOT_WORDS,
    );
  ...
  const navigateWithTransition = useCallback(
    (
      ...
      {
        options,
        words = APP_BOOT_WORDS,
        holdMs = APP_TRANSITION_LOADER_MS,
      } = {},
    ) => {
  ...
  const handleProfileSave = useCallback(
    async (payload) => {
      ...
      const nextSession = {
        ...
        stationMemberships:
          response?.stationMemberships || sessionMeta?.stationMemberships || [],
    ...
    [
      ...
      sessionMeta?.loginAt,
      sessionMeta?.role,
      sessionMeta?.stationMemberships,
    ],
  );
  ...
  const handleFuelArrivalNotification = useCallback(
    async (station, target = {}) => {
      const stationId = String(station?.id || station?.publicId || "").trim();
      if (!stationId) {
        throw new Error("Station is missing a notification id.");
      }

      const nextFavoriteStationIds = mergeStringLists(
        favoriteStationIdsRef.current,
        [stationId],
      );
      setFavoriteStationIds(nextFavoriteStationIds);

      if (stationsApi.isApiMode() && isAuthenticated) {
        await userAuthApi.updatePreferences({
          notifyInApp: true,
          favoriteStationPublicIds: nextFavoriteStationIds,
        });
        remoteFavoriteStationIdsRef.current = nextFavoriteStationIds;
      ...
      if (!notificationsEnabled && isPushSupported()) {
        try {
          ...
          pushMessage =
            error?.message ||
            "Station saved. In-app alerts will still appear in SmartLink.";
        }
      } else if (!isPushSupported()) {
        pushMessage =
          "Station saved. In-app alerts will appear in SmartLink when fuel arrives.";
      }

      const fuelLabel = String(target?.fuelTypeLabel || "fuel").trim() || "fuel";
      return {
        saved: true,
        message:
          pushMessage ||
          `Done. We will notify you when ${fuelLabel.toLowerCase()} arrives.`,
      };
    },
    [handleNotificationsToggle, isAuthenticated, notificationsEnabled],
  );

  const showBrowserNotification = useCallback(
    ({
      notificationId = "",
      title = "SmartLink Alert",
      ...
      if (
        ...
        shownNotificationIdsRef.current.has(scopedNotificationId)
      ) {
        return;
      }
      ...
      const notificationTitle =
        String(title || "SmartLink Alert").trim() || "SmartLink Alert";
      const notificationBody =
        String(message || "").trim() || "You have a new alert.";
  ...
  const showSystemAlertNotification = useCallback(
    (alertPayload) => {
      ...
      showBrowserNotification({
        ...
        title:
          String(alertPayload?.title || "SmartLink Alert").trim() ||
          "SmartLink Alert",
        message:
          String(alertPayload?.message || alertPayload?.body || "").trim() ||
  ...
  const joinQueueFromStation = useCallback(
    async (station, options = {}) => {
      ...
      const stationPublicId = userQueueApi.isApiMode()
        ? String(
            station.publicId || sessionMeta?.station?.publicId || "",
          ).trim()
      ...
      const queueJoinId =
        response?.queueJoinId || response?.status?.queueJoinId;
      if (!queueJoinId) {
  ...
  const getReservationSlotsForStation = useCallback(
    async (station, { fuelType = "PETROL", lookAhead = 8 } = {}) => {
      ...
      return queueData.getReservationSlots(stationPublicId, {
        fuelType,
        lookAhead,
      });
    },
  ...
  const connectReservationSlotRealtime = useCallback(
    (station, handlers = {}) => {
      ...
      if (typeof queueData.connectStationChangesSocket !== "function")
        return () => {};
  ...
  const refreshActiveQueue = useCallback(async () => {
    ...
    try {
      if (queueData === userQueueApi) {
        ...
        const queueJoinId = String(
          payload?.queueJoinId || payload?.status?.queueJoinId || "",
        ).trim();
  ...
  useEffect(() => {
    ...
    const scheduleReconnect = () => {
      ...
      const delay =
        ALERTS_RECONNECT_BACKOFF_MS[
          Math.min(attempt, ALERTS_RECONNECT_BACKOFF_MS.length - 1)
        ];
        ...
        const unsubscribe = userQueueApi.connectUserAlertsSocket({
          ...
          onMessage: (message) => {
            ...
            if (message.type === "user_alert:new") {
              ...
              message.type === "user_alert:read" ||
              message.type === "user_alert:archived"
            ) {
              safeRefreshUnreadAlertsCount();
            }
          },
  ...
  useEffect(() => {
    ...
    const maybeNotifyQueueStatus = (snapshot) => {
      ...
      const queueStatus = String(snapshot.queueStatus || "")
        .trim()
        .toUpperCase();
    ...
    const scheduleReconnect = () => {
      ...
      const delay =
        QUEUE_BG_RECONNECT_BACKOFF_MS[
          Math.min(attempt, QUEUE_BG_RECONNECT_BACKOFF_MS.length - 1)
        ];
    ...
    const connectSocket = () => {
      ...
      try {
        const unsubscribe = userQueueApi.connectQueueSocket({
          ...
          onMessage: (message) => {
            if (!mounted || !message || typeof message !== "object") return;
            const mergedSnapshot = mergeQueueRealtimeSnapshot(
              queueBgSnapshotRef.current,
              message,
            );
            if (mergedSnapshot) {
  ...
  useEffect(() => {
    ...
    const loadRemotePreferences = async () => {
      try {
        ...
        const remoteFavoriteStationIds = normalizeStringList(
          preferences?.favoriteStationPublicIds,
        );
        ...
        if (
          !sameStringList(remoteFavoriteStationIds, mergedFavoriteStationIds)
        ) {
          await userAuthApi
            .updatePreferences({
  ...
  useEffect(() => {
    ...
    if (sameStringList(remoteFavoriteStationIdsRef.current, favoriteStationIds))
      return;
  ...
  const station = routeParamId
    ? stationsData.find((item) => item.id === routeParamId) || null
    : null;
  const favoriteStations = favoriteStationIds
    .map(
      (favoriteId) =>
        stationsData.find((item) => item.id === favoriteId) || null,
    )
    .filter(Boolean);
  ...
  if (showAppLoader) {
    return (
      <section
        ...
        role="status"
        aria-live="polite"
        aria-label="Loading SmartLink app"
      >
        <article className="app-loader-card">
          ...
          <p>
            Loading{" "}
            <span
              key={loaderWords[loaderWordIndex]}
              className="app-loader-word"
            >
  ...
  if (showStationBootstrap && stationsLoading) {
    ...
    screen = (
      <MapHomeScreen
        ...
        onViewStation={(id) =>
          navigateWithTransition(`/m/stations/${id}`, {
            words: APP_STATION_WORDS,
          })
        }
      />
    );
      ...
      <>
        <section className="station-card coming-soon">
          ...
          <button
            ...
            onClick={() => setShowQueueStationPicker(true)}
          >
            Browse Stations
          </button>
        </section>
            ...
            >
              ...
              <div className="queue-station-picker">
                {queuePickerStations.length ? (
                  queuePickerStations.map((item) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        navigateWithTransition(`/m/stations/${item.id}`, {
                          words: APP_STATION_WORDS,
                        });
                      }}
                    >
                  ...
                  <p className="queue-station-picker-empty">
    ...
    screen = (
      <StationDetailsScreen
        ...
        onBack={() => navigate("/m/home")}
        onDirections={(id) =>
          navigateWithTransition(`/m/directions/${id}`, {
            words: APP_ROUTE_WORDS,
          })
        }
        ...
        onNotifyFuelArrival={handleFuelArrivalNotification}
    ...
    screen = (
      <DirectionsScreen
        station={station}
        onBack={() => navigate(`/m/stations/${routeParamId || ""}`)}
      />
    );
    ...
    screen = (
      <WalletScreen
        onOpenSendCredit={() => navigate("/m/wallet/send-credit")}
      />
    );
    ...
    screen = (
      <AssistantScreen layout="mobile" onBack={() => navigate("/m/home")} />
    );
  } else {
    screen = (
      <AccountScreen
  ...
  return (
    <MobileShell
      ...
      onNavigate={navigate}
      showTabBar={
        route.name !== "station-details" &&
        route.name !== "queue-status" &&
        route.name !== "login"
      }
      showAssistantLauncher={
        route.name !== "assistant" && route.name !== "login"
      }
