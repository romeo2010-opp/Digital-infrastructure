...
try {
  ...
  const [supportAssessmentRows] = await db.query(`SELECT a.id assessment_id,a.subject_id,a.name assessment_name,sub.name subject_name
    FROM assessments a
    JOIN subjects sub ON sub.school_id=a.school_id AND sub.id=a.subject_id
    WHERE a.school_id=? AND a.class_id=? AND a.status IN ('published','locked','approved','moderated','results_submitted')
      AND (sub.name LIKE '%Math%' OR sub.name='English' OR sub.name LIKE '%Science%')
    ORDER BY a.id DESC`, [school.id, classRow.id])
  const mathematicsAssessments = supportAssessmentRows.filter((row) => row.subject_name.toLowerCase().includes("math"))
  const englishAssessments = supportAssessmentRows.filter((row) => row.subject_name.toLowerCase() === "english")
  const scienceAssessments = supportAssessmentRows.filter((row) => row.subject_name.toLowerCase().includes("science"))
  const crossSubjectAssessments = [mathematicsAssessments[1], mathematicsAssessments[0], englishAssessments[0], scienceAssessments[0]].filter(Boolean)
  if (crossSubjectAssessments.length < 4) throw new Error("Four approved Mathematics, English and Science assessments are required for the cross-subject support scenario")
  ...
  async function createCase(letter, values, memberIndexes, events, evidence = []) {
    ...
    for (const [index, item] of evidence.entries()) await db.query(`INSERT INTO learner_support_case_evidence (public_ref,school_id,case_id,academic_year_id,term_id,learner_id,subject_id,topic_id,assessment_id,evidence_role,evidence_precision,score_percentage,marks_awarded,marks_available,confidence_score,comparable,comparability_json,evidence_status,observed_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [randomUUID(), school.id, caseId, term.academic_year_id, term.term_id, learner?.id || learners[memberIndexes[0]].id, item.subject_id ?? (values.subject_id === null ? null : subject.id), item.topic_id ?? (values.topic_id === null ? null : topic.id), item.assessment_id || null, item.role || (index ? "reassessment" : "baseline"), item.precision || "question", item.score, item.score === null || item.score === undefined ? null : item.score / 10, item.score === null || item.score === undefined ? null : 10, item.confidence || 85, item.comparable ?? (index ? 1 : 0), j({ comparable: item.comparable ?? true, scenario: letter, assessment_name: item.assessment_name || undefined }), item.status || "valid", days(item.day ?? -20 + index * 10), actor.id, actor.id])
    for (const [index, item] of events.entries()) await db.query("INSERT INTO learner_support_case_events (public_ref,school_id,case_id,term_id,event_type,summary,evidence_json,status,responsible_user_id,linked_entity_type,linked_entity_ref,idempotency_key,occurred_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [randomUUID(), school.id, caseId, term.term_id, item.type, item.summary, j({ scenario: letter, ...(item.evidence || {}) }), item.status || values.status, actor.id, item.linkedType || null, item.linkedRef || null, `greenfield-support-${letter}-${index}`, days(item.day ?? -events.length + index), actor.id, actor.id])
  ...
  const crossSubjectEvidence = crossSubjectAssessments.map((assessment, index) => ({
    ...assessment,
    role: index ? "comparison" : "detection",
    precision: "overall",
    score: [58, 49, 52, 44][index],
    confidence: 80,
    comparable: 1,
    day: [-30, -20, -12, -5][index],
  }))
  const crossSubjectAssessmentNames = crossSubjectEvidence.map((item) => item.assessment_name)
  await createCase("G", { scenario: "G", scope_type: "cross_subject", case_type: "multi_subject_decline", status: "academic_team_review", level: 4, failures: 3, subject_id: null, topic_id: null, summary: `Performance for ${learners[6].name} has declined across Mathematics, English and Science over four valid assessments. A broader academic review is recommended; no cause has been inferred.` }, [6], [{ type: "multi_subject_review_detected", summary: "A neutral cross-subject pattern was identified from valid published evidence.", evidence: { assessment_names: crossSubjectAssessmentNames } }, { type: "academic_review_requested", summary: "The academic team was asked to review the broader learning pattern.", status: "academic_team_review", evidence: { assessment_names: crossSubjectAssessmentNames } }], crossSubjectEvidence)
