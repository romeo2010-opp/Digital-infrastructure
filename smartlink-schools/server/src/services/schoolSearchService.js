...
const DATA_ENTITIES = new Set(["students", "teachers", "guardians", "classes", "subjects", "fees", "discounts", "leave", "payroll", "attendance", "homework", "assessments", "results", "support", "calendar", "messages"])
...
const NAVIGATION_CATALOG = [
  ...
  { id: "payroll", title: "Payroll", subtitle: "Salary runs, payroll items and payslips", route: "/finance/payroll", entity: "payroll", permission: SCHOOL_PERMISSIONS.PAYROLL_VIEW, keywords: "salary wages staff pay" },
...
export async function searchSchoolRecords({ db, schoolId, session, user, teacherClassIds, permissions: permissionList, interpretation, limit = 20 }) {
  ...
  const canPayroll = hasPermission(permissions, SCHOOL_PERMISSIONS.PAYROLL_VIEW)
  ...
  const jobs = {
    ...
    payroll: canPayroll && wants("payroll") ? db.query(`SELECT public_ref id,title,CONCAT(payroll_period_start,' to ',payroll_period_end,' · ',currency,' ',FORMAT(total_net_pay,0)) subtitle,'PAYROLL' resultType,status,CONCAT(title,' ',status,' ',payroll_period_start,' ',payroll_period_end) keywords,CONCAT('/finance/payroll/',public_ref) route FROM payroll_runs WHERE school_id=?${interpretation.primaryState === "pending" ? " AND status IN ('draft','calculated','pending_approval')" : ""} ORDER BY payroll_period_end DESC LIMIT ?`, [schoolId, candidateLimit]) : Promise.resolve([[]]),
