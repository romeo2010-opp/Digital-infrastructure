...
export function buildCalendarSearchScope({ user, teacherClassIds, session } = {}) {
  ...
  const assignmentSessionClause = " AND calendar_assignment.academic_year_id=? AND calendar_assignment.term_id=?"
  params.push(session.academicYearId, session.termId)

  return {
    allowed: true,
    sql: ` AND se.academic_year_id=? AND se.term_id=? AND (
...
export async function searchSchoolRecords({ db, schoolId, session, user, teacherClassIds, permissions: permissionList, interpretation, limit = 20 }) {
  ...
  const teacherSessionReady = !isTeacher || (!setupRequired && Boolean(session?.academicYearId) && Boolean(session?.termId))
  ...
  const assessmentScope = isTeacher
    ? teacherSessionReady
      ? { sql: ` AND a.academic_year_id=? AND a.term_id=? AND EXISTS (SELECT 1 FROM teacher_class_subject_assignments tcsa WHERE tcsa.school_id=a.school_id AND tcsa.teacher_id=? AND tcsa.class_id=a.class_id AND tcsa.subject_id=a.subject_id AND tcsa.academic_year_id=a.academic_year_id AND tcsa.term_id=a.term_id AND tcsa.role='subject_teacher' AND tcsa.is_active=1)`, params: [session.academicYearId, session.termId, user.id] }
      : { sql: " AND 1=0", params: [] }
    : { sql: "", params: [] }
  const resultScope = isTeacher
    ? teacherSessionReady
      ? { sql: ` AND EXISTS (SELECT 1 FROM teacher_class_subject_assignments tcsa WHERE tcsa.school_id=rb.school_id AND tcsa.teacher_id=? AND tcsa.class_id=rb.class_id AND tcsa.subject_id=rb.subject_id AND tcsa.academic_year_id=rb.academic_year_id AND tcsa.term_id=rb.term_id AND tcsa.role='subject_teacher' AND tcsa.is_active=1)`, params: [user.id] }
      : { sql: " AND 1=0", params: [] }
    : { sql: "", params: [] }
  const supportScope = isTeacher
    ? teacherSessionReady
      ? { sql: ` AND lsc.academic_year_id=? AND lsc.current_term_id=? AND EXISTS (SELECT 1 FROM teacher_class_subject_assignments tcsa WHERE tcsa.school_id=lsc.school_id AND tcsa.teacher_id=? AND tcsa.class_id=lsc.class_id AND tcsa.subject_id=lsc.subject_id AND tcsa.academic_year_id=lsc.academic_year_id AND tcsa.term_id=lsc.current_term_id AND tcsa.role='subject_teacher' AND tcsa.is_active=1)`, params: [session.academicYearId, session.termId, user.id] }
      : { sql: " AND 1=0", params: [] }
    : { sql: "", params: [] }
  const subjectScope = isTeacher
    ? teacherSessionReady
      ? { sql: " AND EXISTS (SELECT 1 FROM teacher_class_subject_assignments tcsa WHERE tcsa.school_id=sub.school_id AND tcsa.subject_id=sub.id AND tcsa.teacher_id=? AND tcsa.academic_year_id=? AND tcsa.term_id=? AND tcsa.role='subject_teacher' AND tcsa.is_active=1)", params: [user.id, session.academicYearId, session.termId] }
      : { sql: " AND 1=0", params: [] }
    : { sql: "", params: [] }
  const homeworkTeacherScope = isTeacher
    ? teacherSessionReady
      ? { sql: " AND EXISTS (SELECT 1 FROM teacher_class_subject_assignments tcsa WHERE tcsa.school_id=h.school_id AND tcsa.teacher_id=? AND tcsa.class_id=h.class_id AND tcsa.subject_id=h.subject_id AND tcsa.academic_year_id=? AND tcsa.term_id=? AND tcsa.role='subject_teacher' AND tcsa.is_active=1)", params: [user.id, session.academicYearId, session.termId] }
      : { sql: " AND 1=0", params: [] }
    : { sql: "", params: [] }
