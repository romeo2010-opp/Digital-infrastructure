...
export async function academicCommandCentre(req,res){res.json(await getAcademicCommandCentre(getScopedSchoolId(req),req.query))}
export async function academicOverview(req,res){res.json(await getAcademicCommandCentre(getScopedSchoolId(req),req.query))}
export async function academicClasses(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({classes:data.coverage||[]})}
export async function academicClassDetail(req,res){
  const schoolId=getScopedSchoolId(req)
  const [[row]]=await pool.query('SELECT id,public_ref,name,grade_level FROM classes WHERE school_id=? AND public_ref=? LIMIT 1',[schoolId,String(req.params.classRef||'')])
  if(!row)throw new HttpError(404,'Academic class was not found.')
  const data=await getAcademicCommandCentre(schoolId,{...req.query,class_id:row.id})
  const [topicMatrix,learnerDistribution,upcomingAssessments]=await Promise.all([
    pool.query(`SELECT s.public_ref subject_ref,s.name subject_name,t.public_ref topic_ref,t.topic_name,
      ...
      WHERE ltr.school_id=? AND ams.class_id=? AND ltr.is_official=1
      GROUP BY s.id,s.public_ref,s.name,t.id,t.public_ref,t.topic_name,aec.mastery_threshold
      ORDER BY learners_below_secure DESC,class_result,t.topic_name LIMIT 150`,[schoolId,row.id]),
    pool.query(`SELECT amr.mastery_status,COUNT(DISTINCT amr.student_id) learner_count
      ...
      WHERE amr.school_id=? AND amr.mastery_level='subject' GROUP BY amr.mastery_status`,[row.id,schoolId]),
    pool.query(`SELECT a.id,a.name,a.assessment_type,a.status,a.total_marks,s.public_ref subject_ref,s.name subject_name,ete.exam_date
      ...
      WHERE a.school_id=? AND a.class_id=? AND a.status IN ('open','approved','scheduled')
        AND (ete.exam_date IS NULL OR ete.exam_date>=CURDATE()) ORDER BY COALESCE(ete.exam_date,'9999-12-31'),a.updated_at DESC LIMIT 20`,[schoolId,row.id]),
  ])
  res.json({
...
export async function academicSubjects(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);const subjects=new Map();for(const row of data.coverage||[]){const current=subjects.get(row.subject_name)||{subject_name:row.subject_name,coverage:[],class_count:0};current.coverage.push(row);current.class_count+=1;subjects.set(row.subject_name,current)}res.json({subjects:[...subjects.values()]})}
export async function academicSubjectDetail(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);const subject=String(req.params.subjectRef||'');res.json({subject_ref:subject,coverage:(data.coverage||[]).filter((row)=>row.subject_ref===subject||row.subject_name===subject),risks:data.alerts||[],recommendations:data.recommendations||[]})}
export async function academicTopicDetail(req,res){
  const schoolId=getScopedSchoolId(req)
  const topicRef=String(req.params.topicRef||'')
  const [[topic]]=await pool.query(`SELECT st.public_ref,st.topic_name,st.description,st.order_number,s.public_ref subject_ref,s.name subject_name FROM syllabus_topics st JOIN subjects s ON s.id=st.subject_id AND s.school_id=st.school_id WHERE st.school_id=? AND st.public_ref=? LIMIT 1`,[schoolId,topicRef])
  if(!topic)throw new HttpError(404,'Academic topic was not found.')
  const [evidence,data]=await Promise.all([
    getCanonicalAcademicEvidence(schoolId,{...req.query,topic_ref:topicRef,limit:req.query.limit||100}),
    getAcademicCommandCentre(schoolId,req.query),
  ])
...
export async function academicEvidence(req,res){res.json(await getCanonicalAcademicEvidence(getScopedSchoolId(req),req.query))}
export async function academicRisks(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({risks:data.alerts||[]})}
export async function academicInsights(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({insights:[...(data.recommendations||[]),...(data.alerts||[])].slice(0,100)})}
export async function academicPositiveSignals(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({positive_signals:data.positive_signals||[]})}
export async function academicMeaningfulChanges(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({meaningful_changes:data.meaningful_changes||[]})}
export async function academicEvidenceGaps(req,res){
  ...
  const evidenceGaps=[
    ...totalsOnly[0].map((row)=>({...row,gap_type:'assessment_totals_only',action:'map or diagnose',confidence:'aggregate only',reason:'Published totals support aggregate performance only. Topic diagnosis is unavailable until mapped question or topic evidence is recorded.'})),
    ...unmapped[0].map((row)=>({...row,gap_type:'questions_without_topic_mappings',action:'map questions',confidence:'limited',reason:`${Number(row.unmapped_count)} of ${Number(row.question_count)} questions have no valid topic allocation, so their marks cannot support precise topic findings.`})),
    ...taughtNotAssessed[0].map((row)=>({...row,gap_type:'taught_not_assessed',action:'create diagnostic',confidence:'unknown',reason:'Teaching is recorded, but no assessment evidence yet confirms learner understanding.'})),
    ...staleEvidence[0].map((row)=>({...row,gap_type:'no_recent_evidence',action:'schedule evidence',confidence:'stale',reason:row.last_activity?'No mapped marks have been published in the last 45 days.':'No published mapped assessment evidence is available for this class and subject.'})),
    ...missingMarks[0].map((row)=>({...row,gap_type:'missing_marks',action:'finish marking',confidence:'provisional',reason:`The mark sheet is ${Number(row.completion_percentage||0).toFixed(0)}% complete and remains excluded from official mastery.`})),
    ...incompleteInterventions[0].map((row)=>({...row,gap_type:'incomplete_intervention',action:row.gap_state==='overdue'?'escalate':'schedule reassessment',confidence:'pending outcome',reason:row.gap_state==='overdue'?'The intervention review date has passed without a measured outcome.':'The active intervention has no linked reassessment plan.'})),
  ]
...
export async function academicReadiness(req,res){const data=await getAcademicCommandCentre(getScopedSchoolId(req),req.query);res.json({readiness:data.readiness||[]})}
export async function academicHistory(req,res){res.json(await getAcademicIntelligenceHistory(getScopedSchoolId(req),req.query))}
export async function academicExplanation(req,res){res.json(await getAcademicFindingExplanation(getScopedSchoolId(req),String(req.params.findingId||'')))}
...
export async function studentAcademicIntelligence(req,res){res.json(await getStudentAcademicIntelligence(getScopedSchoolId(req),String(req.params.studentRef||'')))}
