...
test("Classroom Mode prioritises the active timetable subject over stale lessons", async () => {
  ...
  assert.match(service, /teacher_lesson_logs[\s\S]*lessonTimetableEntryId,lessonDate/)
  assert.match(service, /REFERENCED_TABLE_NAME referenced_table/)
  assert.match(service, /constraint\?\.referenced_table[\s\S]*timetable_entries/)
  const timetableMigration = await readFile(new URL("../database/064_lesson_log_timetable_scope.sql", import.meta.url), "utf8")
  assert.match(timetableMigration, /UPDATE teacher_lesson_logs lesson[\s\S]*referenced_table_name='exam_timetable_entries'[\s\S]*SET lesson\.timetable_entry_id=NULL[\s\S]*WHERE lesson\.timetable_entry_id IS NOT NULL/)
  assert.doesNotMatch(timetableMigration, /LEFT JOIN timetable_entries/)
  assert.match(service, /classroom_sessions[\s\S]*timetableEntryId/)
  assert.match(service, /role='subject_teacher'[\s\S]*academic_year_id=\?[\s\S]*term_id=\?/)
  assert.doesNotMatch(service, /academic_year_id IS NULL|term_id IS NULL/)
