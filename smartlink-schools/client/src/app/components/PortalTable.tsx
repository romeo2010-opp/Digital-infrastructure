...
export function PortalTable({
  ...
  return (
    <div className="flex min-h-0 w-full flex-col overflow-hidden">
      <div
        ref={topScrollRef}
        className={`shrink-0 overflow-x-auto overflow-y-hidden border-b border-[var(--mera-panel-border-soft)] bg-[var(--mera-panel-muted)] ${hasHorizontalOverflow ? 'h-3' : 'hidden'}`}
        ...
        <table ref={tableRef} data-slot="table" className="w-full min-w-max caption-bottom text-sm">
          ...
          <tr data-slot="table-row" className="border-b transition-colors">
            {columns.map((column) => (
              ...
              className={`sticky top-0 z-20 h-10 whitespace-nowrap border-b border-[var(--mera-panel-border-soft)] bg-[#fafafa] px-4 text-left align-middle text-[12px] font-medium text-[var(--mera-panel-text-muted)] shadow-[0_1px_0_0_var(--mera-panel-border-soft)] [[data-mera-density=compact]_&]:h-8 [[data-mera-density=compact]_&]:px-3 [[data-mera-density=compact]_&]:text-[11px] ${column.className || ''}`}
              >
                {column.label}
              </th>
            ))}
          ...
          {rows.length ? (
            rows.map((row, index) => (
              <tr
                ...
                data-slot="table-row"
                className={`border-b border-[var(--mera-panel-border-soft)] transition-colors ${onRowClick ? 'cursor-pointer hover:bg-[#fafafa]' : ''}`}
                onClick={onRowClick ? () => onRowClick(row) : undefined}
              >
                {columns.map((column) => (
                  <td key={column.key} data-slot="table-cell" className="whitespace-nowrap px-4 py-3 align-middle text-[13px] font-normal text-[var(--mera-panel-text-soft)] [[data-mera-density=compact]_&]:px-3 [[data-mera-density=compact]_&]:py-2 [[data-mera-density=compact]_&]:text-[12px]">
