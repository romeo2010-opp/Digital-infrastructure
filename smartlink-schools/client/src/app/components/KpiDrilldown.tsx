...
export function KpiDrilldownCard({
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2">
            <span className="size-2 rounded-full ring-4 ring-black/5" style={{ color: topAccent, background: topAccent }} />
            <div className="truncate text-[12px] font-medium tracking-[0] text-[var(--mera-panel-text-muted)]">{label}</div>
          </div>
          <div className="mt-2 truncate text-[22px] font-medium leading-none tracking-[0] text-[var(--mera-panel-text)]">{renderDrilldownValue(value)}</div>
        </div>
        {onClick ? <ArrowRight className="mt-0.5 size-4 shrink-0 text-[var(--mera-panel-border-strong)] transition group-hover:text-[var(--mera-panel-text-soft)]" /> : null}
      </div>
      <div className="mt-3 flex min-w-0 items-center gap-2">
        {delta ? <span className={`rounded-[999px] px-2 py-1 text-[11px] font-medium leading-none ${toneClass}`}>{renderDrilldownValue(delta)}</span> : null}
        {helper ? <span className="min-w-0 truncate text-[11px] font-normal text-[var(--mera-panel-text-muted)]">{renderDrilldownValue(helper)}</span> : null}
      </div>
    </>
  )
  ...
  return onClick ? (
    <button
      type="button"
      onClick={onClick}
      className="group relative min-h-[96px] overflow-hidden rounded-[8px] border border-[var(--mera-panel-border)] bg-[var(--mera-panel)] px-4 py-3 text-left shadow-[var(--mera-shadow-card)] transition hover:-translate-y-0.5 hover:border-[#d4d4d4] hover:shadow-[0_7px_18px_rgba(0,0,0,0.08)] focus:outline-none focus:ring-2 focus:ring-[#6bdd9e]/25"
    >
      {content}
    </button>
  ) : (
    <article className="relative min-h-[96px] overflow-hidden rounded-[8px] border border-[var(--mera-panel-border)] bg-[var(--mera-panel)] px-4 py-3 text-left shadow-[var(--mera-shadow-card)]">
      {content}
    </article>
  )
}
