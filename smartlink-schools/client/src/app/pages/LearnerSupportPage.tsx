  ...
  return <main className="grid gap-3 p-4">
    ...
    <div className="grid gap-3 xl:grid-cols-[1.25fr_.75fr]">
      <div className="grid gap-3">
        <SectionCard title="Case timeline" subtitle="Evidence and human decisions remain attached across terms"><div className="space-y-0 p-4">{(data.timeline || []).map((item: any) => <article key={item.public_ref} className="relative border-l-2 border-[#cbd5e1] pb-5 pl-4 last:pb-0"><span className="absolute -left-[5px] top-1 size-2 rounded-full bg-indigo-600"/><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-[11px] font-semibold capitalize text-[#111827]">{label(item.event_type)}</h3><time className="text-[9px] text-[#64748b]">{new Date(item.occurred_at).toLocaleString()}</time></div><p className="mt-1 text-[10px] leading-5 text-[#64748b]">{item.summary}</p>{item.assessment_name ? <p className="mt-1 text-[9px] font-medium leading-4 text-indigo-700">Assessments: {item.assessment_name}</p> : null}<div className="mt-1 flex gap-2"><Badge value={item.status}/>{item.responsible_user ? <span className="text-[9px] text-[#64748b]">{item.responsible_user}</span> : null}</div></article>)}{!data.timeline?.length ? <p className="text-[11px] text-[#64748b]">No timeline events have been recorded.</p> : null}</div></SectionCard>
        <SectionCard title="Evidence history" subtitle="Published, status-valid evidence and the detection it produced stay together">
          <div className="overflow-x-auto"><table className="w-full text-left text-[10px]"><thead className="border-b bg-[#f8fafc] text-[#64748b]"><tr><th className="p-3">Date</th><th>Evidence</th><th>Assessment</th><th>Topic</th><th>Precision</th><th>Score</th><th>Confidence</th><th>Comparable</th></tr></thead><tbody>{(data.evidence || []).map((item: any) => <tr key={item.public_ref} className="border-b align-top last:border-0"><td className="p-3 whitespace-nowrap">{date(item.observed_at)}</td><td className="min-w-52 py-3 pr-3"><strong className="font-semibold text-[#111827]">{label(item.event_type || item.evidence_role)}</strong>{item.summary ? <p className="mt-1 max-w-sm leading-4 text-[#64748b]">{item.summary}</p> : null}</td><td className="min-w-56 py-3 pr-3">{item.assessment_name || 'Assessment not linked'}</td><td className="py-3 pr-3">{item.topic_name || (item.event_type === 'multi_subject_review_detected' ? 'Cross-subject' : 'Limited')}</td><td className="py-3 pr-3"><Badge value={item.evidence_precision}/></td><td className="py-3 pr-3">{item.score_percentage === null ? '—' : `${Number(item.score_percentage).toFixed(1)}%`}</td><td className="py-3 pr-3">{Number(item.confidence_score).toFixed(0)}%</td><td className="py-3 pr-3"><Badge value={item.evidence_kind === 'case_event' ? 'pattern' : item.comparable ? 'comparable' : 'not comparable'}/></td></tr>)}</tbody></table></div>
        </SectionCard>
