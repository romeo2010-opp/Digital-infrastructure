...
function SupportCaseDetail({ caseRef }: { caseRef: string }) {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const [caseResult, timelineResult, evidenceResult, setupResult, assessmentResult] = await Promise.allSettled([
        api.getSupportCase(token, caseRef),
        api.getSupportTimeline(token, caseRef),
        api.getSupportEvidence(token, caseRef),
        api.getAcademicAuthoringSetup(token),
        api.listTargetedAssessments(token, {}),
      ])
      if (caseResult.status === 'rejected') throw caseResult.reason

      setData(caseResult.value)
      setTimeline(timelineResult.status === 'fulfilled' ? timelineResult.value?.timeline || [] : [])
      setEvidence(evidenceResult.status === 'fulfilled' ? evidenceResult.value?.evidence || [] : [])
      setSetup(setupResult.status === 'fulfilled' ? setupResult.value || { classes: [] } : { classes: [] })
      setAssessments(assessmentResult.status === 'fulfilled' ? assessmentResult.value?.assessments || [] : [])

      const unavailable: string[] = []
      if (timelineResult.status === 'rejected') unavailable.push('timeline')
      if (evidenceResult.status === 'rejected') unavailable.push('evidence history')
      if (setupResult.status === 'rejected') unavailable.push('assessment setup')
      if (assessmentResult.status === 'rejected') unavailable.push('targeted assessments')
      if (unavailable.length) setError(`This case loaded, but ${unavailable.join(', ')} could not be loaded. The available case information remains usable.`)
  ...
  const hasTargetedAssessmentScope = Boolean(record.class_ref && record.subject_ref && record.topic_ref && record.case_type !== 'multi_subject_decline')
  const caseAssessments = hasTargetedAssessmentScope ? assessments.filter((item: any) => item.class_ref === record.class_ref && item.subject_ref === record.subject_ref && (item.topic_ref === record.topic_ref || item.subtopic_ref === record.topic_ref)) : []
  ...
  return <main className="grid gap-3 p-4">
    ...
    {tab === 'reassessment' ? <div className="grid gap-3"><SectionCard title="Reassessment state" subtitle="A published result triggers the canonical intervention outcome evaluation"><div className="divide-y divide-[#edf0f4]">{(data.reassessments || []).map((item: any) => <article key={item.public_ref} className="grid gap-3 p-4 md:grid-cols-[1fr_140px_180px]"><div><div className="flex gap-2"><Badge value={item.outcome}/><Badge value={item.assessment_status}/></div><h3 className="mt-2 text-[12px] font-semibold">{item.assessment_title || 'Targeted reassessment'}</h3></div><div className="text-[10px]"><strong className="block">Due</strong>{date(item.due_at)}</div><div>{item.assessment_id ? <Button variant="outline" className="h-8" onClick={() => navigate(`/results/${item.assessment_id}`)}>Open linked mark sheet</Button> : <span className="text-[10px] text-[#64748b]">Publish the assessment to create its mark sheet.</span>}</div></article>)}{!data.reassessments?.length ? <div className="p-6 text-center text-[11px] text-[#64748b]">No reassessment has been linked yet.</div> : null}</div></SectionCard>
      {actions.has('create_assessment') && hasTargetedAssessmentScope ? <TargetedAssessmentWorkflow setup={setup} supportCase={record} rows={caseAssessments} onRefresh={load}/> : null}
