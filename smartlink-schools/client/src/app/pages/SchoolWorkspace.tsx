...
} from "../components/SchoolActionModal";
...
function actionForPage(pageKey: SchoolPageKey): SchoolActionKind {
  ...
  if (
    pageKey === "assessmentInsights" ||
    pageKey === "dailyDrill" ||
    pageKey === "examForecast"
  )
    return "insights";
  return "report";
}
...
function buildKpis(pageKey: SchoolPageKey, rows: any[]) {
  if (pageKey === "classes") {
    const studentTotal = rows.reduce(
      (sum, row) => sum + Number(row.rawStudentCount || 0),
      0,
    );
    return [
      {
      ...
      },
      {
        label: "Learners",
        value: studentTotal,
        helper: "inside classes",
        delta: "visible scope",
        tone: "good" as const,
      },
      {
        label: "Assigned Teachers",
        value: rows.filter((row) => row.teacher !== "Unassigned").length,
        helper: "classes",
        delta: "staff scope",
      },
      {
        label: "Empty Classes",
        value: rows.filter((row) => Number(row.rawStudentCount || 0) === 0)
          .length,
        helper: "classes",
        delta: "review",
      },
  ...
  if (pageKey === "fees") {
    const outstanding = rows.reduce(
      (sum, row) => sum + Number(row.rawBalance || 0),
      0,
    );
    return [
      {
      ...
      },
      {
        ...
        helper: "database total",
        delta: "review queue",
        tone: outstanding > 0 ? ("warn" as const) : ("good" as const),
      },
      {
        label: "Paid",
  ...
  if (pageKey === "attendance") {
    ...
    return [
      ...
      },
      {
        ...
        value: present,
        helper: "today",
        delta: marked ? percent((present / marked) * 100) : "0.0%",
        tone: "good" as const,
      },
      {
        label: "Late",
        value: rows.filter((row) => row.status === "late").length,
        helper: "today",
        delta: "review",
        tone: "warn" as const,
      },
      {
        label: "Absent",
        value: rows.filter((row) => row.status === "absent").length,
        helper: "today",
        delta: "alert",
        tone: "bad" as const,
      },
  ...
  if (pageKey === "assessmentInsights") {
    return [
      ...
      },
      {
        ...
        value: rows.reduce((sum, row) => sum + Number(row.rawSupport || 0), 0),
        helper: "learners",
        delta: "teacher queue",
      },
    ];
  ...
  return [
    {
      label: "Records",
      value: rows.length,
      helper: "database rows",
      delta: "live data",
    },
    {
      label: "Visible Scope",
      value: rows.length ? "Active" : "Empty",
      helper: "role based",
      delta: "database",
    },
  ];
...
function mapRows(pageKey: SchoolPageKey, payload: any) {
  ...
  if (pageKey === "messages") {
    return (payload?.messages || []).map((row: any) => ({
      ...
      audience:
        row.audience_label ||
        (row.recipient_scope?.type === "school"
          ? "Whole school"
          : "Selected classes"),
  ...
  if (pageKey === "results" || pageKey === "examBuilder") {
    return (payload?.results || []).map((row: any) => ({
      ...
      examSession:
        ...
        (row.assessment_type ? statusLabel(row.assessment_type) : "-"),
      className: row.class_name,
      subject: row.subject_name,
      termName: row.term_name,
  ...
  if (pageKey === "examForecast") {
    return (payload?.forecasts || []).map((row: any) => ({
      ...
      action:
        Number(row.priority_score || 0) >= 70 ? "Revise first" : "Practice set",
    }));
  }
  if (pageKey === "reports") {
...
async function loadPage(api: any, token: string, pageKey: SchoolPageKey) {
  ...
  if (pageKey === "results" || pageKey === "examBuilder")
    return api.listResults(token);
  if (pageKey === "assessmentInsights")
    return api.listAssessmentInsights(token);
...
export function SchoolWorkspace({
      ...
      if (payload?.setup_required) {
        setError(
          payload?.message ||
            ...
            "Set up an active academic year and term to view current records.",
        );
      }
    } catch (err: any) {
      setRows([]);
  ...
  useEffect(() => {
    ...
    api
      .quickSearch(token, value, 20, controller.signal)
  ...
  if (!page) {
    return (
      <div className="grid gap-3 p-4">
        <SectionCard
          title="School Search"
          subtitle="Search students, parents, classes, homework, results and messages"
        >
          <div className="grid gap-3 p-4">
            <Toolbar>
              ...
              <button
                type="button"
                onClick={() => openAction("filters")}
                className="inline-flex h-8 items-center gap-2 rounded-[5px] border border-[#e2e8f0] bg-white px-3 text-[12px] font-semibold text-[#374151]"
              >
            ...
            <PortalTable
              ...
              emptyMessage={
                query.trim().length < 2
                  ? "Type at least two characters to search school records."
                  : searchLoading
  ...
  return (
    <div className="grid gap-3 p-4">
      <section className="rounded-[6px] border border-[var(--mera-panel-border)] bg-[var(--mera-panel)] p-4 shadow-[var(--mera-shadow-card)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {canUsePrimaryAction ? (
            <button
              type="button"
              onClick={() => openAction(pageAction)}
              className="inline-flex h-8 items-center gap-2 rounded-[5px] bg-[#111827] px-3 text-[12px] font-semibold text-white"
            >
        ...
        <SectionCard
          ...
          subtitle={
            loading
              ...
              : `${visibleRows.length} database records represented`
          }
        >
          <PortalTable
            ...
            emptyMessage={
              loading ? "Loading records..." : "No database records available."
            }
          />
        </SectionCard>
      ...
      {pageKey === "classes" ? (
        <TeacherAssignmentsPanel onChanged={refresh} />
      ) : null}
