...
import { subscribeUserAlerts } from "./userAlertsHub.js"

const WS_PATH = "/ws/user-alerts"
...
export async function attachUserAlertsWebSocket(server) {
  let wsRuntime = null
  try {
    wsRuntime = await import("ws")
  } catch {
    // eslint-disable-next-line no-console
    console.warn("[realtime] ws dependency not installed. User alerts websocket disabled.")
  ...
  wss.on("connection", async (ws, req) => {
    ...
    try {
      ...
      unsubscribe = subscribeUserAlerts(userId, (message) => {
        safeSend(ws, message)
      })

      safeSend(ws, {
        type: "user_alert:ready",
