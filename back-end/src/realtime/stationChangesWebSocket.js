...
const WS_PATH = "/ws/station-changes"
...
export async function attachStationChangesWebSocket(server) {
  let wsRuntime = null
  try {
    wsRuntime = await import("ws")
  } catch {
    // eslint-disable-next-line no-console
    console.warn("[realtime] ws dependency not installed. WebSocket change feed disabled.")
  ...
  wss.on("connection", async (ws, req) => {
    ...
    try {
      ...
      unsubscribe = subscribeStationChanges(stationId, (message) => {
        safeSend(ws, message)
      })
